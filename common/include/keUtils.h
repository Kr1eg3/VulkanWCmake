...
static uint32_t findMemoryType(VkPhysicalDevice vkPhysDevice, uint32_t typeFilter, VkMemoryPropertyFlags properties) {
...
static void createBuffer(VkDevice vkDevice, VkPhysicalDevice vkPhysDevice, VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory) {
	...
	allocInfo.memoryTypeIndex = findMemoryType(vkPhysDevice, memRequirements.memoryTypeBits, properties);
...
static bool LoadObj(attrib_t *attrib, std::vector<shape_t> *shapes,
